	...
	for {
		...
		default:
			...
			if powBuffer.SetBytes(result).Cmp(target) <= 0 {
				...
				logStr := fmt.Sprintf("-------\n[timeStamp: %v] mined potential block, number: %d, nonce: %d, attempts: %d, result: %d, target: %v, difficulty: %d\n\n",
					time.Now().Unix(), number, nonce, attempts, powBuffer.SetBytes(result).Uint64(), target, header.Difficulty.Uint64())
