...
const (
	repuWindowSize = 10000
	E              = 2.718281828459054523
	weight         = 0.5
)

var (
	genesisDifficult = big.NewInt(0)
)
...
func (ethash *Ethash) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header, uncle bool, seal bool, unixNow int64) error {
	...
	expected := chain.GetHeaderByNumber(0).Difficulty

	repuFloat := ethash.calcReputation(chain, header, parent)
	repuVal := repuFloat * float64(expected.Uint64()) * weight
	repuBig := big.NewInt(int64(repuVal))
	expected = new(big.Int).Sub(expected, repuBig)
...
// calcReputation calculate miner's reputation according to it's history block
func (ethash *Ethash) calcReputation(chain consensus.ChainHeaderReader, header, parent *types.Header) float64 {
	...
	log.Info("[currentHeader]: ", "currentHeader Number", *chain.CurrentHeader().Number)

	bigOne := big.NewInt(1)

	/*
		if _, ok := ethash.lastMinedBlockNumber[header.Coinbase]; !ok {
			ethash.lastMinedBlockNumber[header.Coinbase] = big.NewInt(0)
		}

		if parent.Number.Cmp(ethash.lastMinedBlockNumber[header.Coinbase]) > 0 {
			hNumber := ethash.historyBlockNumber[header.Coinbase]
			i := ethash.lastMinedBlockNumber[header.Coinbase]
			i.Add(i, bigOne)
			for ; i.Cmp(parent.Number) <= 0; i.Add(i, bigOne) {
				blockNumber := i.Uint64()
				if chain.GetHeaderByNumber(blockNumber).Coinbase == header.Coinbase {
					hNumber++
				}
				ethash.lastMinedBlockNumber[header.Coinbase] = i
			}
			ethash.historyBlockNumber[header.Coinbase] = hNumber
		}
	*/

	recentHistoryNum := 0
	windowSize := 0
	cur := chain.CurrentHeader().Number
	for i := big.NewInt(0); i.Cmp(parent.Number) <= 0 && windowSize < repuWindowSize && i.Cmp(cur) <= 0; i = i.Add(i, bigOne) {
		blockNumber := i.Uint64()
		log.Info("[BlockNumber]: ", "miner", header.Coinbase, "number", blockNumber)
		if chain.GetHeaderByNumber(blockNumber).Coinbase == header.Coinbase {
			recentHistoryNum++
		}
		windowSize++
	...
	log.Info("[Repu]: ", "miner", header.Coinbase, " recentBlockNumber", recentHistoryNum)

	var repu float64
	defer func() {
		logStr := fmt.Sprintf("----[repu]: blockNumber[%d], miner: [%v], historyBlockNumber[%d], difficulty: [%d], repu: [%f]\n", header.Number.Uint64(), header.Coinbase, recentHistoryNum, header.Difficulty.Uint64(), repu)
		filePath := "/root/gowork/src/repu-core-geth/privateChain/run_log"
		file, err := os.OpenFile(filePath, os.O_WRONLY|os.O_APPEND, 0666)
		if err != nil {
			log.Info("open file err")
		}
		defer file.Close()
	...
	if recentHistoryNum > 0 {
		base := -1 * 0.2 * float64(recentHistoryNum)
		num := rmath.Pow(E, base+5)
		repu = 1 / (1 + num)
	}

	if repu > 0.0000000000 {
		return repu
	}
...
func (ethash *Ethash) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	if genesisDifficult.Cmp(common.Big0) == 0 && parent.Number.Cmp(common.Big0) == 0 {
		genesisDifficult = parent.Difficulty
	}

	header.Difficulty = chain.GetHeaderByNumber(0).Difficulty
	repuFloat := ethash.calcReputation(chain, header, parent)
	repuVal := repuFloat * float64(header.Difficulty.Uint64()) * weight
	repuBig := big.NewInt(int64(repuVal))
	log.Info("[RepuBig]", "miner", header.Coinbase, "repuBig", repuBig.Uint64())
	header.Difficulty = new(big.Int).Sub(header.Difficulty, repuBig)
